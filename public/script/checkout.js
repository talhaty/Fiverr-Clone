          ...
          for (let i in data["department"]) {
            if(jobTitle == data["gigTitle"][i]){
              let jobListItem = `
                          ...
                          <img class="card-img-top" src=${data["image"][i]["url"]} alt="Card image cap" style="width:60rem; height:30rem">
                          <div class="card-body">
                            <h5 class="card-title">${jobTitle}</h5>
                            <p class="card-text">${data["about"][i]}</p>
                            <p class="card-text"><small class="text-muted">Price: ${data["price"][i]}$ </br> <b>Quiz Percentage: ${data["lastAttempt"][i]['percentage']}%</b> </small></p>
